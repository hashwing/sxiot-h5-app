...
var hui = (function(selector, undefined){
	...
	var hcExtends = {
		...
		setSwitchVal : function(val){
			var span = this.dom[0].getElementsByTagName('span');
			if(val){
				this.addClass("hui-switch-on");
				span[0].innerHTML = "on";
			}else{
				this.removeClass("hui-switch-on");
				span[0].innerHTML = "off";
			}
		...
		ranged : function(callBack){this.dom[0].onchange = function(){callBack(this.value);}},
		setRangeVal : function(val){this.val(val)},
	...
	huibase.ajax = function(sets){
		...
		if (sets.header!=''){
			xhr.setRequestHeader('Auth', sets.header);
		}
		if(sets.type == 'POST'){
			var pd = '';
	...
	huibase.getSxito = function(url,header,success, err){
		var sets = {url:url, type:'GET', backType:'JSON', success:success, error:err,header:header};
		this.ajax(sets);
	};
	huibase.postSxito = function(url,header,pd,success, err){
		var sets = {url:url, type:'POST', backType:'JSON', data:pd, success:success, error:err,header:header};
		this.ajax(sets);
	};
